...
- **Tool schema `oneOf`/`allOf`/`anyOf` are not supported at the top level of `inputSchema` by the Claude API** (returns 400). These combinators were removed from `connect-schema.ts` and `add-element-schema.ts`. The equivalent constraints are preserved as prose in the tool `description` and individual property `description` fields — the runtime handler validation still enforces them. Nested uses inside `properties` (e.g. `anyOf` inside a property definition) remain valid.
